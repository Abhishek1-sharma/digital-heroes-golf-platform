...
Create a `.env.local` file in the project root by copying `.env.example`, then fill in the public values from your Supabase project settings:
...
Traditional golf membership platforms are transactional and aesthetically outdated. Play Golf explores a different model: what if a monthly golf subscription also entered you into a reward draw, contributed to a charity you chose, and gave you a leaderboard presence worth caring about?
...
3. setup_scores.sql
4. setup_profile_trigger.sql
5. setup_draw_engine.sql
6. setup_winner_lifecycle.sql
7. setup_donations.sql
8. setup_admin_scores.sql
9. fix_admin_policies.sql
10. fix_leaderboard_rls.sql
11. setup_contract_fixes.sql
12. setup_submission_hardening.sql
13. setup_access_hardening.sql
14. fix_profiles_rls.sql
```
...
| `VITE_STRIPE_MONTHLY_PRICE_ID` | Stripe recurring monthly Price ID  |
| `VITE_STRIPE_YEARLY_PRICE_ID`  | Stripe recurring yearly Price ID   |
...
Play Golf demonstrates what a modern subscription web platform looks like when built thoughtfully — with real business logic, a layered security model, and a design that respects the user's intelligence. Every feature in this README is implemented, working, and exercisable through the live demo.
...
_Built with React, TypeScript, Supabase, and Tailwind CSS. Deployed on Vercel._
