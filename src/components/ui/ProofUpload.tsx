...
const ProofUpload: React.FC<ProofUploadProps> = ({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const { error: proofError } = await supabase.rpc("submit_winner_proof", {
        p_draw_id: drawId,
      ...
      setError(
        err.message ||
          "Failed to upload proof. Ensure the file is a standard image.",
      );
    } finally {
      setUploading(false);
    }
      ...
      {!file ? (
        ...
        <div className="space-y-4">
          <div className="h-32 border border-white/10 rounded-xl flex flex-col items-center justify-center bg-white/5 relative overflow-hidden">
            ...
            <span className="text-sm font-medium z-10 px-4 text-center truncate w-full">
              {file.name}
            </span>
            <span className="text-[10px] text-on-surface-variant z-10">
          ...
          >
            {uploading ? (
              ...
              <>
                <Check className="w-4 h-4" /> Submit Proof
              </>
            )}
          </button>
        </div>
