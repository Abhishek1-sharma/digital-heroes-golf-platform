...
const AdminOverview: React.FC = () => {
  ...
  const fetchAdminStats = async () => {
    ...
    try {
      ...
      const totalRaised =
        allCharities?.reduce((acc, c) => acc + (c.total_raised || 0), 0) || 0;
      const totalPrizes =
        allDraws?.reduce((acc, d) => acc + (d.prize_pool || 0), 0) || 0;
    ...
    <div className="max-w-[1600px] mx-auto p-12">
      ...
      <div className="mb-20">
        ...
        <p className="text-xl text-on-surface-variant font-sans max-w-2xl leading-relaxed">
          Administrative oversight for the distributed Play Golf network.
          Accessing real-time Matrix metrics, member health, and charity
          priority.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
        {[
          ...
          },
          {
            ...
            icon: Heart,
            color: "text-primary",
            sub: "Across 12 Charities",
          },
          {
            label: "Tiered Payouts",
            value: formatCurrency(stats.totalPrizes),
            icon: Trophy,
            color: "text-secondary",
            sub: stats.publishedDraws + " Protocols Executed",
          },
        ].map((metric, idx) => (
          <motion.div
            key={idx}
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <p
                className={cn(
                  "text-3xl font-display font-black tracking-tight mb-2",
                  metric.color,
                )}
              >
                {metric.value}
          ...
          <div className="glass-card p-10 bg-surface-container-high/20">
            ...
            <div className="space-y-6">
              {recentWinners.length === 0 ? (
                <p className="text-xs text-on-surface-variant opacity-50 italic py-10 text-center">
                  No protocol winners found in the Matrix.
                </p>
              ) : (
                ...
                <div key={idx} className="space-y-3">
                  <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest">
                    ...
                    <span className="text-primary">
                      {Math.round(
                        (charity.total_raised / stats.totalRaised) * 100,
                      )}
                      %
                    </span>
                  </div>
                  <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${(charity.total_raised / stats.totalRaised) * 100}%`,
                      }}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
        <div className="glass-card p-8 flex items-center gap-6 group cursor-pointer hover:bg-primary/5 transition-all">
          ...
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest mb-1 group-hover:text-primary transition-colors">
              Users
            </h4>
            <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest">
              Manage Matrix Entities
            </p>
          </div>
        ...
        <div className="glass-card p-8 flex items-center gap-6 group cursor-pointer hover:bg-secondary/5 transition-all">
          ...
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest mb-1 group-hover:text-secondary transition-colors">
              Subscriptions
            </h4>
            <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest">
