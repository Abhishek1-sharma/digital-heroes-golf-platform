  ...
  const fetchProofs = async () => {
    try {
      ...
      if (userIds.length > 0 && drawIds.length > 0) {
        const [{ data: entries }, { data: profiles }, { data: draws }] =
          await Promise.all([
            supabase
              .from("draw_entries")
              .select("user_id, draw_id, winner_status")
              .in("user_id", userIds)
              .in("draw_id", drawIds),
            supabase
              .from("profiles")
            ...
            supabase
              .from("draws")
              .select("id, draw_month, prize_pool")
              .in("id", drawIds),
          ]);

        const proofsWithUrls = await Promise.all(
          proofsWithStatus.map(async (proof) => {
            const path = proof.file_path || proof.file_url;
            const { data: signedFile } = await supabase.storage
              .from("winner-proofs")
              .createSignedUrl(path, 60 * 60);
            return {
              ...proof,
              file_url: signedFile?.signedUrl || proof.file_url,
            };
          }),
        );
        proofsWithStatus = proofsWithUrls;

        proofsWithStatus =
          proofsWithUrls.map((proof) => {
            const entry = entries?.find(
              (e) =>
                e.user_id === proof.user_id &&
                e.draw_id === proof.draw_id &&
                e.winner_status !== "none" &&
                e.winner_status !== "pending",
            );
    ...
    try {
      if (newStatus !== "paid") {
        const { error } = await supabase
          ...
          .update({
            status: newStatus,
            review_note: reviewNotes[proof.id] || null,
      ...
      const entryResult =
        newStatus === "paid"
          ? await supabase.rpc("mark_winner_paid", {
              p_user_id: proof.user_id,
              p_draw_id: proof.draw_id,
            })
          : await supabase
              .from("draw_entries")
  ...
  return (
    <div className="max-w-[1600px] mx-auto p-12">
      ...
      <p className="text-on-surface-variant max-w-2xl mb-12">
        Review and verify algorithmic draw winner proofs (scorecards). Approved
        payouts will be automatically dispatched.
      </p>
      ...
      <h3 className="text-xl font-display font-bold uppercase tracking-tight mb-6 mt-12 flex items-center gap-3">
        <Clock className="w-5 h-5 text-secondary" /> Pending Verification (
      ...
      {pendingProofs.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {pendingProofs.map((proof) => (
            <div
              key={proof.id}
              className="glass-card border border-secondary/30 overflow-hidden flex flex-col"
            >
              <div className="h-48 bg-surface-container-highest relative group">
                ...
                <div className="absolute inset-0 bg-background/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center hover:bg-primary/20 hover:text-primary transition-colors backdrop-blur-md text-white"
                  >
              ...
              <div className="p-6 flex-grow flex flex-col">
                ...
                <textarea
                  ...
                  rows={3}
                  className="mb-6 w-full resize-none rounded-xl border border-white/10 bg-background/40 p-3 text-xs outline-none focus:border-primary"
                />
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-surface-container-high/30">
              <th className="px-8 py-5 text-[10px] font-bold uppercase tracking-[0.2em] text-on-surface-variant">
                Entity
              </th>
              <th className="px-8 py-5 text-[10px] font-bold uppercase tracking-[0.2em] text-on-surface-variant">
                Draw
              ...
              <th className="px-8 py-5 text-[10px] font-bold uppercase tracking-[0.2em] text-on-surface-variant text-right">
                Date
              </th>
              <th className="px-8 py-5 text-[10px] font-bold uppercase tracking-[0.2em] text-on-surface-variant text-right">
                Action
            ...
            {processedProofs.map((proof) => (
              ...
              >
                ...
                <td className="px-8 py-6">
                  ...
                  >
                    {proof.actual_status === "pending_verification"
                      ? "pending"
                      : proof.actual_status || proof.status}
                  </span>
                </td>
