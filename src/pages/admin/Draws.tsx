...
import { SCORE_MAX, SCORE_MIN } from "../../lib/platform";
...
const AdminDraws: React.FC = () => {
  ...
  const handleSimulate = async () => {
    ...
    setMessage(null);
    try {
      const numericCustom = customNumbers
        .map((n) => parseInt(n))
        .filter((n) => !isNaN(n));
      if (
        manualOverride &&
        (numericCustom.length !== 5 ||
          new Set(numericCustom).size !== 5 ||
          numericCustom.some(
            (number) => number < SCORE_MIN || number > SCORE_MAX,
          ))
      ) {
        throw new Error(
          `Enter five unique numbers between ${SCORE_MIN} and ${SCORE_MAX}.`,
        );
      }
      const finalCustom = manualOverride ? numericCustom : undefined;
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "Draw successfully published to the Matrix!",
      });
  ...
  const handleArchive = async (drawId: string) => {
    const { error } = await supabase.rpc("archive_draw", { p_draw_id: drawId });
    if (error) {
      setMessage({ type: "error", text: error.message });
      return;
    }
    setMessage({ type: "success", text: "Draw archived successfully." });
    loadMetaData();
  };
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12 mb-20">
        <div className="max-w-3xl">
          ...
          <p className="text-xl text-on-surface-variant font-sans max-w-2xl">
            Orchestrate the algorithmic selection process. Manage prize pools,
            matching logic, and official draw publishing.
          </p>
        </div>
              ...
              <div className="p-8 bg-background/50 rounded-3xl border border-white/5 space-y-6">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-on-surface-variant">
                    Monthly Contribution Pool
                  </span>
                  <span className="font-bold">Calculated on simulation</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-on-surface-variant">
                    Jackpot Rollover
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xl font-display font-bold text-primary">
                    {simulationResult ? formatCurrency(simulationResult.totalPool) : "Run simulation"}
                  </span>
                </div>
              </div>
              ...
              {drawHistory.map((draw) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="text-[9px] font-bold uppercase tracking-widest opacity-50 mb-1">
                      {draw.draw_mode}
                    </p>
                    <div className="flex gap-1 justify-end">
                      {draw.winning_numbers.slice(0, 3).map((n, i) => (
                        <div
                          key={i}
                          className="w-4 h-4 bg-primary/20 rounded-sm flex items-center justify-center text-[8px] text-primary"
                        >
                          {n}
                        </div>
                      ))}
                    ...
                    {draw.status === "published" && (
                      <button
                        onClick={() => handleArchive(draw.id)}
                        className="mt-3 text-[9px] font-bold uppercase tracking-widest text-secondary hover:text-primary"
                      >
                        Archive draw
                      </button>
                    )}
            ...
            <div className="glass-card h-full flex flex-col items-center justify-center p-20 text-center space-y-8 border-dashed border-2 border-white/5 opacity-50">
              ...
              <div>
                ...
                <p className="text-on-surface-variant max-w-sm">
                  Run a Matrix simulation to preview winning numbers, matching
                  metrics, and prize allocations.
                </p>
              </div>
            </div>
          )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <div className="glass-card p-10">
                    ...
                    <p className="text-4xl font-display font-bold tracking-tighter">
                      {formatCurrency(simulationResult.totalPool)}
                    </p>
                  </div>
                  <div className="glass-card p-10">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-6">
                    ...
                    <p className="text-4xl font-display font-bold tracking-tighter">
                      {simulationResult.eligibleCount}{" "}
                      <span className="text-lg text-on-surface-variant font-sans">
                        / {simulationResult.participantsCount}
                      </span>
                    </p>
                  </div>
                  <div className="glass-card p-10">
                    ...
                    <p
                      className={cn(
                        ...
                        simulationResult.newRollover > 0
                          ? "text-secondary"
                          : "text-primary",
                      )}
                    >
                ...
                <div className="glass-card overflow-hidden">
                  <div className="p-10 border-b border-white/5 flex items-center justify-between">
                    <h3 className="text-sm font-bold uppercase tracking-[0.2em] flex items-center gap-3">
                      <Trophy className="w-5 h-5 text-secondary" /> Simulation
                      Entities
                    </h3>
                  </div>
                      ...
                      <tbody className="divide-y divide-white/5">
                        {simulationResult.winners.length === 0 ? (
                          <tr>
                            <td
                              colSpan={3}
                              className="px-10 py-20 text-center text-on-surface-variant opacity-50 italic"
                            >
                              No winning entities in this simulation sequence.
                            </td>
                          </tr>
                        ) : (
                          simulationResult.winners.map(
