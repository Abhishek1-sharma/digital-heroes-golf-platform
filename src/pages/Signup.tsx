  ...
  const onSubmit = async (data: SignupFormValues) => {
    ...
    try {
      ...
      if (authData.user) {
        ...
        // means Supabase does not return a session yet.
        if (authData.session) {
          navigate("/onboarding");
        } else {
          setSuccess(true);
      ...
      if (message.toLowerCase().includes("rate limit")) {
        setError(
          "Supabase email limit reached. Wait a few minutes, use a different test email, or disable email confirmation in Supabase Auth settings for local testing.",
        );
      } else {
      ...
      <div className="min-h-screen flex items-center justify-center bg-background px-6 pt-20 relative overflow-hidden">
        ...
        >
          ...
          <p className="text-xl text-on-surface-variant font-sans max-w-md mx-auto mb-12 leading-relaxed">
            Your entry into the Play Golf matrix is being processed. Please
            check your email to verify your account.
          </p>
          <Link
            to="/login"
            className="inline-block w-full py-6 text-[10px] bg-primary text-background font-black uppercase tracking-[0.3em] rounded-2xl hover:scale-[1.02] transition-all shadow-2xl shadow-primary/20 active:scale-95 text-center"
          >
            Return to Login
          </Link>
        </motion.div>
