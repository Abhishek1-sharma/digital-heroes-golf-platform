...
const Profile: React.FC = () => {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "Protocol updated: Identity sync complete.",
      });
    } catch (err) {
      console.error(err);
      setMessage({
        type: "error",
        text: "Update failed: Signal interference detected.",
      });
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="mx-auto max-w-3xl p-8 md:p-16">
        <div className="glass-card p-10 text-center">
          <AlertCircle className="mx-auto mb-5 h-10 w-10 text-amber-400" />
          <h1 className="mb-4 text-2xl font-display font-black uppercase">
            Identity unavailable
          </h1>
          <p className="text-sm leading-relaxed text-on-surface-variant">
            Your account is authenticated, but its profile row could not be
            loaded. Run the profile repair SQL or contact an administrator.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex items-center gap-6 mb-16">
        ...
        <div>
          <div className="flex items-center gap-4">
            <h1 className="text-5xl font-display font-black uppercase tracking-tight">
          ...
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-on-surface-variant flex items-center gap-3">
            Core Protocol Settings / Account 0x
            ...
            <div className="flex items-center gap-2 mb-2">
              <div className="flex-grow h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div
                  className={cn(
                    "h-full bg-primary",
                    profile.role === "admin" ? "w-full" : "w-1/2",
                  )}
                />
              </div>
              <span className="text-[10px] font-bold text-primary">
                {profile.role === "admin" ? "Alpha" : "Beta"}
              </span>
            </div>
            <p className="text-[8px] text-on-surface-variant uppercase tracking-widest leading-loose">
              Access permissions verified and synced with primary core.
            </p>
          </div>
        </div>
              ...
              <div className="space-y-4 pt-10 border-t border-white/5 font-sans">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[
                    ...
                    {
                      name: "Matrix Base Identity",
                      status: "Verified",
                      color: "text-primary",
                    },
                    {
                      name: "GHIN Direct Connect",
                      status: "Inactive",
                      color: "text-on-surface-variant",
                    },
                    {
                      name: "Neural 2FA Encryption",
                      status: "Verified",
                      color: "text-primary",
                    },
                  ].map((item, idx) => (
                    <div
                      key={idx}
                      className="p-5 rounded-2xl border border-white/5 bg-white/5 flex items-center justify-between group"
                    >
