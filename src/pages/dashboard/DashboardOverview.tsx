...
import CharityImage from "../../components/charity/CharityImage";
import type { Score, Charity } from "../../types";
import { MAX_RETAINED_SCORES, SCORE_MAX, SCORE_MIN } from "../../lib/platform";
...
const DashboardOverview: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    setDataError(null);
    try {
      const [scoreResult, charityResult, drawResult] = await Promise.all([
        supabase
          .from("scores")
          .select("*")
          .eq("user_id", user?.id)
  ...
  const handleScoreSubmit = async (e: React.FormEvent) => {
    ...
    if (!isPremium) {
      setMessage({
        type: "error",
        text: "Premium membership required to submit scores.",
      });
      return;
    }

    const points = parseInt(newScore.points);
    if (isNaN(points) || points < SCORE_MIN || points > SCORE_MAX) {
      setMessage({
        type: "error",
        text: `Points must be between ${SCORE_MIN} and ${SCORE_MAX}`,
      });
      return;
    }
    ...
    try {
      const { data: currentScores, error: scoreFetchError } = await supabase
        .from("scores")
        .select("id, date")
        .eq("user_id", user.id)
      ...
      if (
        currentScores?.some(
          (score) => score.date === new Date().toISOString().split("T")[0],
        )
      ) {
        throw new Error(
          "A score already exists for today. Edit or delete the existing entry first.",
        );
      }

      if (currentScores && currentScores.length >= MAX_RETAINED_SCORES) {
        const idsToDelete = currentScores
          .slice(0, currentScores.length - (MAX_RETAINED_SCORES - 1))
          .map((score) => score.id);
  ...
  const activeCharity = charities.find(
    (c) => c.id === (subscription?.charity_id || profile?.selected_charity_id),
  );
  ...
  return (
    <>
      <div className="max-w-[1600px] mx-auto p-12">
        {dataError && (
          <div className="mb-8 flex items-start justify-between gap-4 rounded-2xl border border-red-500/30 bg-red-500/10 p-5 text-sm text-red-300">
            <p>Dashboard data could not be loaded: {dataError}</p>
            <button
              onClick={fetchData}
              className="font-bold uppercase tracking-widest text-xs hover:text-white"
            >
              Retry
            </button>
          </div>
            ...
            <div className="flex flex-wrap items-center gap-6">
              ...
              <div className="flex items-center gap-4 bg-surface-container-low px-6 py-3 rounded-2xl border border-white/5 shadow-xl">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-display font-bold uppercase text-secondary">
                    {latestDraw
                      ? formatCurrency(latestDraw.prize_pool)
                      : "£---,---"}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 justify-end">
                <span
                  className={cn(
                    ...
                    isActive
                      ? "bg-primary shadow-[0_0_15px_rgba(78,222,163,0.5)]"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                    <div>
                      ...
                      <div className="flex gap-3">
                        {latestEntry ? (
                          latestEntry.entry_numbers.map(
                            (n: number, i: number) => (
                              <div
                                key={i}
                                className="w-12 h-12 bg-surface-container-low border border-white/5 rounded-xl flex items-center justify-center font-display font-bold text-lg hover:border-primary/30 transition-colors"
                              >
                                {n}
                              </div>
                            ),
                          ...
                          <div className="flex items-center gap-2 text-on-surface-variant text-[10px] font-bold uppercase tracking-widest px-4 py-3 bg-white/5 rounded-xl border border-dashed border-white/10">
                            {isPremium ? (
                              "Entry Pending Next Draw"
                            ) : (
                              <>
                                <Lock className="w-3 h-3" /> Subscription
                                Required
                              </>
                            )}
                          </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {latestEntry.prize_amount > 0 && (
                            <span className="text-xl font-display font-bold text-secondary">
                              +{formatCurrency(latestEntry.prize_amount)}
                            </span>
                          )}
                        </div>
                      ) : (
                        <p className="text-xs opacity-40 italic">
                          Waiting for official protocol...
                        </p>
                      )}
                    </div>
                    <div>
                      ...
                      <p className="text-4xl font-display font-black text-primary tracking-tighter">
                      ...
                      >
                        <div className="bg-background rounded-[1.4rem] p-6 flex items-center justify-between hover:bg-surface-container-low transition-colors">
                          <div className="flex items-center gap-5">
                            ...
                            <div>
                              <h4 className="font-display font-bold text-sm uppercase tracking-tight">
                                {score.course_name}
                              </h4>
                  ...
                  {!isPremium && (
                    <div className="absolute inset-0 z-50 bg-background/60 backdrop-blur-sm flex items-center justify-center p-10 text-center">
                      <div className="max-w-xs">
                        ...
                        <h4 className="text-xl font-display font-black uppercase mb-4 tracking-tight">
                          Access Restricted
                        </h4>
                        <p className="text-sm text-on-surface-variant mb-8 leading-relaxed">
                          Upgrade your membership to start posting scores and
                          enter monthly draws.
                        </p>
                        <Link
                          to="/dashboard/subscription"
                          className="inline-block px-8 py-3 bg-primary text-background rounded-full font-bold uppercase tracking-widest text-[10px] hover:scale-105 transition-all active:scale-95"
                        >
                          Upgrade Now
                        </Link>
                      </div>
                  ...
                  <div className="bg-surface-container-low/50 rounded-[2.4rem] p-10">
                    ...
                    <form onSubmit={handleScoreSubmit} className="space-y-6">
                      <div className="space-y-3">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewScore({ ...newScore, points: e.target.value })
                          }
                          placeholder="e.g. 38"
                          disabled={!isPremium}
                      ...
                      <div className="space-y-3">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewScore({ ...newScore, course: e.target.value })
                          }
                    ...
                    <div className="mt-10 p-5 bg-background/30 rounded-2xl border border-white/5 flex gap-4">
                      ...
                      <p className="text-[10px] text-on-surface-variant leading-relaxed font-sans">
                        Your entry will be verified against official club
                        records. Fraudulent submissions result in immediate
                        account termination.
                      </p>
                    </div>
                  </div>
          ...
          <div className="space-y-10">
            ...
            <div className="glass-card p-1 rounded-[2.5rem] overflow-hidden group">
              <div className="bg-background rounded-[2.4rem] overflow-hidden flex flex-col">
                <div className="h-40 relative overflow-hidden">
                  <CharityImage
                    src={
                      activeCharity?.logo_url || "/images/charity-fallback.svg"
                    }
                    alt={activeCharity?.name || "Charity partner"}
                ...
                <div className="p-10 -mt-16 relative z-10">
                  {activeCharity ? (
                    <>
                      <div className="w-20 h-20 bg-surface-container-low rounded-3xl border border-white/10 p-3 mb-6 shadow-xl">
                        <CharityImage
                          src={activeCharity.logo_url}
                          alt={`${activeCharity.name} logo`}
                    ...
                    <div className="py-8 text-center">
                      ...
                      <h3 className="text-lg font-display font-bold uppercase mb-2 tracking-tight">
                        No Charity Selected
                      </h3>
                      <p className="text-xs text-on-surface-variant mb-6 leading-relaxed">
                        Pick a cause to direct your subscription contribution
                        toward.
                      </p>
                    </div>
                  )}
                  <Link
                    ...
                    className="w-full py-4 rounded-full border border-white/10 text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-white/5 transition-all active:scale-95"
                  >
            ...
            <div className="glass-card p-1 rounded-[2.5rem]">
              <div className="bg-surface-container-low rounded-[2.4rem] p-10 border border-secondary/20">
                ...
                <Link
                  to="/dashboard/subscription"
                  className="w-full py-4 rounded-full border border-secondary text-secondary text-[10px] font-bold uppercase tracking-widest hover:bg-secondary/5 transition-all active:scale-95 flex items-center justify-center mb-6"
                >
                  Manage Plan
                </Link>
                <p className="text-[9px] text-center text-on-surface-variant font-sans uppercase tracking-widest">
                  Cancel anytime. Rewards are only eligible for active
                  subscribers.
                </p>
              </div>
            </div>
        ...
        <div className="mt-20 space-y-12">
          <div className="flex flex-col md:flex-row justify-between items-end gap-8 px-2">
            <div>
              <p className="text-on-surface-variant text-sm font-sans max-w-xl">
                Explore these hand-selected partners pushing the boundaries of
                global change. Your next performance could be their
                breakthrough.
              </p>
            </div>
            <Link
              to="/charities"
              className="text-primary font-bold uppercase tracking-widest text-[10px] flex items-center gap-3 hover:gap-5 transition-all"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {showcaseCharities.length > 0 ? (
              showcaseCharities.map((charity, idx) => (
                <motion.div
                  key={charity.id}
                ...
                >
                  <Link
                    ...
                    className="bg-background rounded-[2.4rem] overflow-hidden block h-full group"
                  >
                    ...
                    <div className="p-8">
                      ...
                      <h4 className="font-display text-xl font-bold mt-3 mb-2 uppercase tracking-tight">
                        {charity.name}
                      </h4>
                      <p className="text-on-surface-variant text-xs mb-6 line-clamp-2 leading-relaxed">
                        {charity.description}
                      </p>
