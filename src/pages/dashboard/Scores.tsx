...
const scoreSchema = z.object({
  ...
  stableford_points: z.coerce
    .number()
    .min(SCORE_MIN, `Points must be at least ${SCORE_MIN}`)
    .max(SCORE_MAX, `Max Stableford points is ${SCORE_MAX}`),
});
...
const Scores: React.FC = () => {
  ...
  const onSubmit = async (formData: ScoreFormValues) => {
    ...
    setError(null);
    try {
      // The database function enforces subscription state, duplicate dates and
      // rolling-five retention even when somebody bypasses this UI.
      const { error: saveError } = await supabase.rpc("save_my_score", {
        p_course_name: formData.course_name,
        p_date: formData.date,
        p_stableford_points: formData.stableford_points,
  ...
  const avgScore =
    scores.length > 0
      ? (
          scores.reduce((acc, s) => acc + s.stableford_points, 0) /
          scores.length
        ).toFixed(1)
      : "0";
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-8 lg:p-12">
      ...
      <p className="text-on-surface-variant mb-12">
        Track your latest 5 rounds. Your draw entry capability is calculated
        based on the average of your maximum 5 active scores.
      </p>
        ...
        <div className="lg:col-span-1 space-y-8">
          <div className="glass-card p-8 bg-surface-container-high/30 relative overflow-hidden">
            {!isPremium && (
              <div className="absolute inset-0 z-50 bg-background/60 backdrop-blur-sm flex items-center justify-center p-8 text-center border border-dashed border-primary/20">
                <div className="flex flex-col items-center">
                  ...
                  <h4 className="text-sm font-display font-black uppercase mb-2 tracking-tight">
                    Locked
                  </h4>
                  <Link
                    to="/dashboard/subscription"
                    className="text-[9px] font-bold uppercase text-primary border-b border-primary/30"
                  >
                    Connect to Matrix
                  </Link>
                </div>
            ...
            <h3 className="text-sm font-bold uppercase tracking-[0.2em] mb-6 flex items-center gap-3">
              <Plus className="w-4 h-4 text-primary" /> {editingId ? "Edit Score" : "Log New Score"}
            </h3>
            ...
            >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled={!isPremium}
                    className="w-full bg-surface-container-low border border-white/5 rounded-xl py-3 pl-12 pr-4 text-sm focus:border-primary transition-colors outline-none disabled:opacity-30"
                    placeholder={`Points (${SCORE_MIN}-${SCORE_MAX})`}
                  />
                </div>
                {errors.stableford_points && (
                  <p className="text-red-500 text-[10px] mt-1">
                    {errors.stableford_points.message}
                  </p>
                )}
              </div>
              ...
              >
                {submitting ? (
                  ...
                  editingId ? "Save Score" : "Submit Score"
                )}
              </button>
            </form>
          </div>
          ...
          <div className="glass-card p-8 border-primary/20 relative overflow-hidden">
            ...
            <div className="flex items-end gap-2 relative z-10">
              ...
              <span className="text-sm font-bold text-on-surface-variant mb-1">
                pts
              </span>
            </div>
                    ...
                    <div
                      ...
                      className={cn(
                        ...
                        step <= scores.length
                          ...
                          : "bg-white/5 text-on-surface-variant opacity-30",
                      )}
                    >
                      {step}
                    </div>
                ...
                {scores.map((score, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-10">
                      ...
                      <div>
                        <h4 className="font-display font-bold text-xl uppercase tracking-tight mb-1 group-hover:text-primary transition-colors">
                          {score.course_name}
                        </h4>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-4 py-2 rounded-xl bg-primary/10 text-primary text-[9px] font-bold uppercase tracking-widest hover:bg-primary/20 transition-all opacity-0 group-hover:opacity-100"
                      >
                        Edit
                      </button>
              ...
              <div className="p-8 bg-primary/5 border-t border-white/5 flex items-center justify-center gap-4">
                ...
                <span className="text-[10px] text-on-surface-variant font-bold uppercase tracking-[0.2em] leading-relaxed">
                  ...
                  <span className="text-primary">
                    {5 - scores.length} more rounds
                  </span>{" "}
                  to achieve maximum entry weight for the next draw.
                </span>
              </div>
            )}
            ...
            {scores.length === 5 && (
              <div className="p-8 bg-secondary/10 border-t border-secondary/20 flex items-center justify-center gap-4">
                ...
                <span className="text-[10px] text-secondary font-bold uppercase tracking-[0.2em] leading-relaxed">
                  Maximum protocol capacity reached. New entries will cycle
                  existing nodes.
                </span>
              </div>
            )}
